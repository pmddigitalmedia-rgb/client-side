...
import { MobileGalleryFastScroller } from './components/MobileGalleryFastScroller';
...
import { editImageWeather, stringifyError, enhancePrompt, LuxuryMarketingPack, generateDawnToDuskVideo, generateSunnySkiesVideo, generateFurnitureBuildVideo, generateCustomVideo, generateVideoFromImage, generateSurgicalMask } from './services/geminiService';
import { resizeAndProcessImage, convertPdfToImage, getImageDimensions, createZipArchive, processImageForApi, applyWatermarkToDataUrl, normalizeImage, cropToRatio, sharpenImage, reprojectRectilinearTo360, project360ToRectilinear, dilateMask, extractVideoThumbnailAndDimensions } from './services/imageUtils';
import { WEATHER_PRESETS, PANORAMA_PRESETS, VISUAL_STAGER_PRESETS, FURNITURE_STYLES, STAGING_ROOMS } from './constants';
import { ImageItem, WeatherPreset, GeneratedAsset, SocialAssets, ImageItemConfig, PropertyData } from './types';
...
function App() {
  ...
  const { user, profile, isAdmin, isActualAdmin, clientPreviewMode, setClientPreviewMode, deductCredits } = useAuth();
  ...
  // Fallback to studio if client is currently on admin-only property-website
  useEffect(() => {
    if (!isAdmin && activeTab === 'property-website') {
      setActiveTab('studio');
    }
  }, [isAdmin, activeTab]);
  ...
  const handleToolToggle = (toolId: string, specificItemId?: string) => {
    const targetId = specificItemId || activeItemId;
    if (!targetId) return;
  ...
  const handleSyncTools = () => {
    ...
    setCustomConfirm({
      ...
      cancelText: "Cancel",
      onConfirm: () => {
        const toolsToSync = [...finalSource.assignedTools];
    ...
    try {
      ...
      let combinedPrompts = activePresets.map(preset => {
          ...
          if (preset.id === 'furniture' || preset.id === 'p360_vstaging_3d') {
            ...
            p = p.replace('{style}', item.config.swapStyle || FURNITURE_STYLES[0]);
      ...
      const isSunnySkiesTask = activePresets.some(p => p.id === 'sunny_skies' || p.id === 'p360_sunny_skies') ||
      ...
      const TRANSFORMATIVE_TOOLS = ['furniture', 'p360_vstaging_3d', 'style_swapper', 'p360_style_swap', 'auto_declutter', 'p360_auto_declutter', 'sunset', 'lush_lawn', 'snow_removal', 'floor_replacer', 'ceiling_replacer', 'cable_remover'];
      ...
      if (hasStyleSwap || (!hasEmptyRoom && !hasWallUnifier && activePresets.some(p => TRANSFORMATIVE_TOOLS.includes(p.id)))) {
  ...
  const handleGenerate = async (forceOverride = false) => {
    ...
    // Token Guard Safety Cap Interception Check
    if (!forceOverride && tokenGuardSettings.enabled && !tokenGuardSettings.overriddenToday) {
    ...
    if (!isAdmin) {
      ...
      // Calculate total credits needed: 3 credits for videos/panos, 1 credit for standard tools
      let totalCreditsNeeded = 0;
      targets.forEach(t => {
        const hasPanoOrVideo = t.is360 || t.assignedTools.some(tool => tool.includes('video') || tool.startsWith('p360_'));
        totalCreditsNeeded += hasPanoOrVideo ? 3 : Math.max(1, t.assignedTools.length);
      });
    ...
    const generationTasks = targets.map(async (item) => {
      ...
      const hasEmpty = item.assignedTools.includes('empty_room');
  ...
  const handleBatchDownload = async () => {
    ...
    try {
      const results = await Promise.all(selectedTargets.map(async (item, index) => {
        ...
        // Export in original aspect ratio
        const processedUrl = sourceUrl;
        const targetRatio = item.dimensions ? item.dimensions.width / item.dimensions.height : 3/2;
        ...
        // Master Print version (Ultra High Resolution 4000px+, 300 DPI, High Fidelity 0.98 quality)
        ...
        // Web version (High-Definition Web Gallery 2048px max width, 72 DPI, 0.88 quality)
  ...
  const handleDownloadSingle = async (item: ImageItem, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (currentAsset?.type === 'video') {
          ...
          // Export in master print resolution
          const processedUrl = sourceUrl;
          const dims = await getImageDimensions(processedUrl);
          ...
          const printW = item.is360 ? 6144 : Math.max(dims.width, originalW, 4000);
          const printH = Math.round(printW / targetRatio);
          const blob = await resizeAndProcessImage(processedUrl, printW, printH, item.file.type || 'image/jpeg', false, true, 300, 0.98);
    ...
    toolConfig?: any
     ...
     try {
        ...
        if (preset.id === 'furniture' || preset.id === 'p360_vstaging_3d') {
          ...
          prompt += ' [STRICT_WALL_COLOR_LOCK]: Absolutely DO NOT change, repaint, tint, or modify any wall colors, accent walls, or wall paint finishes. Keep all wall colors and textures 100% identical to the source image. [STRICT_STRUCTURAL_PRESERVATION]: Zero added walls, zero moved walls. Preserve all original architectural geometry 100% unchanged. Only place freestanding furniture, rugs, and decor on existing floor space.';
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-100 pb-20 bg-slate-950 relative">
      <Header 
      ...
      {isActualAdmin && clientPreviewMode && (
        <div className="bg-gradient-to-r from-cyan-950/95 via-slate-900 to-cyan-950/95 border-b border-cyan-500/30 px-4 py-2 text-xs text-cyan-200 flex flex-wrap items-center justify-between gap-3 shadow-lg z-30 relative animate-fade-in">
      ...
      <div className="sticky top-24 z-40 bg-slate-950/60 backdrop-blur-xl border-b border-white/5 py-4">
        <div className="max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-8">
            <div className="glass-panel p-2 rounded-2xl flex flex-wrap items-center justify-between gap-3 border border-white/10 shadow-2xl">
                <div className="flex bg-slate-900/80 p-1 rounded-xl border border-white/5">
                    ...
                    <button onClick={() => setActiveTab('social')} className={`px-4 sm:px-5 py-2 rounded-lg text-xs font-bold uppercase transition-all ${activeTab === 'social' ? (isAdmin ? 'bg-orange-500 text-white shadow-lg' : 'bg-cyan-500 text-slate-950 font-black shadow-lg') : 'text-slate-500 hover:text-slate-300'}`}>Social Kit</button>
                    <button onClick={() => setActiveTab('ai-analyst')} className={`px-4 sm:px-5 py-2 rounded-lg text-xs font-bold uppercase transition-all ${activeTab === 'ai-analyst' ? (isAdmin ? 'bg-orange-500 text-white shadow-lg' : 'bg-cyan-500 text-slate-950 font-black shadow-lg') : 'text-slate-500 hover:text-slate-300'}`}>AI Analyst</button>
                    {isAdmin && (
      ...
      <main className="flex-grow max-w-[1440px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-4">
        {activeTab === 'studio' && (
          <div className="flex flex-col lg:flex-row gap-6 items-start">
              {items.length === 0 ? (
                ...
                <>
                  <aside className="lg:w-[350px] lg:shrink-0 lg:sticky lg:top-[160px] glass-panel p-5 space-y-6 rounded-[1.5rem] shadow-2xl max-h-[calc(100vh-200px)] overflow-y-auto scrollbar-hide">
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">Studio Tools</label>
                        </div>
                      ...
                      <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">Visual Stager</label>
                        <div className="grid grid-cols-3 gap-2">
                            {VISUAL_STAGER_PRESETS.map((preset) => {
                              const isAssigned = activeItem?.assignedTools.includes(preset.id);
                              return (
                                <button key={preset.id} draggable onDragStart={(e) => handleDragStartTool(e, preset.id)} onClick={() => handleToolToggle(preset.id)} className={`flex flex-col items-center justify-center p-3 rounded-2xl border aspect-square transition-all cursor-grab active:cursor-grabbing ${isAssigned ? `${preset.color} scale-105 shadow-xl` : 'bg-slate-900/50 border-slate-800 text-slate-500 hover:bg-slate-800'}`}>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-5 h-5 mb-1"><path strokeLinecap="round" strokeLinejoin="round" d={preset.icon} /></svg>
                                    <span className="text-[7px] text-center font-bold uppercase tracking-tighter">{preset.label}</span>
                                </button>
                              );
                      ...
                      <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest block">360 Panoramas</label>
                        <div className="grid grid-cols-2 gap-2">
                            {PANORAMA_PRESETS.map((preset) => {
                              const isAssigned = activeItem?.assignedTools.includes(preset.id);
                              return (
                                <button key={preset.id} draggable onDragStart={(e) => handleDragStartTool(e, preset.id)} onClick={() => handleToolToggle(preset.id)} className={`relative flex items-center gap-3 p-3 rounded-2xl border transition-all cursor-grab active:cursor-grabbing ${isAssigned ? preset.color : 'bg-slate-900/50 border-slate-800 text-slate-500 hover:bg-slate-800'}`}>
                                    ...
                                    {(preset.hasAiWatermark || preset.id === 'p360_style_swap') && (
                                      <span className="ml-auto px-1.5 py-0.5 bg-amber-500 text-[6.5px] font-black text-slate-950 rounded uppercase tracking-tight flex items-center gap-1 shadow-sm" title="Includes MLS-compliant AI watermark">
                                        <svg className="w-1.5 h-1.5 shrink-0" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2L14.5 9.5L22 12L14.5 14.5L12 22L9.5 14.5L2 12L9.5 9.5L12 2Z"/></svg>
                                        AI
                                      </span>
                                    )}
                      ...
                      {activeItem && activeItem.assignedTools.length > 0 && (
                        <div className="p-4 rounded-2xl border bg-slate-900/40 border-white/5 space-y-4 animate-fade-in-up">
                           {activeItem.assignedTools.includes('custom_edit') && (
                             <div className="space-y-3">
                           ...
                           {activeItem.assignedTools.includes('custom_video') && (
                             <div className="space-y-3">
                           ...
                           {(activeItem.assignedTools.includes('style_swapper') || activeItem.assignedTools.includes('p360_style_swap')) && (
                             <div className="space-y-4">
                               <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Replacement Style</label><select value={activeItem.config.swapStyle} onChange={(e) => updateActiveItemConfig({ swapStyle: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-orange-500 appearance-none cursor-pointer">{FURNITURE_STYLES.map(style => (<option key={style} value={style}>{style}</option>))}</select></div>
                               <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Swap Detail</label><input type="text" value={activeItem.config.swapDescriptor} onChange={(e) => updateActiveItemConfig({ swapDescriptor: e.target.value })} placeholder="e.g. replace sectional with wing chairs" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-orange-500" /></div>
                               <div className="flex items-center gap-2 p-2 rounded-lg bg-emerald-950/40 border border-emerald-500/20 text-emerald-400">
                                 ...
                                 <div className="text-[9px] font-bold leading-tight">
                                   ...
                                   <span className="block text-[8px] font-normal text-emerald-300/80">No added walls, moved walls, or room alterations</span>
                                 </div>
                                </div>
                                <div className="flex items-center gap-2 p-2 rounded-lg bg-amber-950/40 border border-amber-500/20 text-amber-400">
                                  ...
                                  <div className="text-[9px] font-bold leading-tight">
                                    ...
                                    <span className="block text-[8px] font-normal text-amber-300/80">MLS-compliant "ai virtually staged" disclosure watermark applied</span>
                                  </div>
                                </div>
                             </div>
                           )}
                           {(activeItem.assignedTools.includes('furniture') || activeItem.assignedTools.includes('p360_vstaging_3d') || activeItem.assignedTools.includes('furniture_build_video')) && (
                             <div className="space-y-4 border-t border-white/5 pt-4">
                               <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Room Type</label><select value={activeItem.config.stagingRoom} onChange={(e) => updateActiveItemConfig({ stagingRoom: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-orange-500 appearance-none cursor-pointer">{STAGING_ROOMS.map(room => (<option key={room} value={room}>{room}</option>))}</select></div>
                               <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Staging Style</label><select value={activeItem.config.stagingStyle} onChange={(e) => updateActiveItemConfig({ stagingStyle: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-orange-500 appearance-none cursor-pointer">{FURNITURE_STYLES.map(style => (<option key={style} value={style}>{style}</option>))}</select></div>
                               <div className="space-y-2"><label className="text-[9px] font-black text-slate-500 uppercase tracking-widest block">Stage Detail</label><input type="text" value={activeItem.config.stageDescriptor} onChange={(e) => updateActiveItemConfig({ stageDescriptor: e.target.value })} placeholder="Type extra details..." className="w-full bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-xs text-slate-300 outline-none focus:ring-1 focus:ring-orange-500" /></div>
                               <div className="flex items-center gap-2 p-2 rounded-lg bg-emerald-950/40 border border-emerald-500/20 text-emerald-400">
                                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 flex-shrink-0 text-emerald-400"><path fillRule="evenodd" d="M10 1a4.5 4.5 0 00-4.5 4.5V9H5a2 2 0 00-2 2v6a2 2 0 002 2h10a2 2 0 002-2v-6a2 2 0 00-2-2h-.5V5.5A4.5 4.5 0 0010 1zm3 8V5.5a3 3 0 10-6 0V9h6z" clipRule="evenodd" /></svg>
                                 <div className="text-[9px] font-bold leading-tight">
                                   <span className="uppercase tracking-wider block">Wall & Architecture Lock: Active</span>
                                   <span className="block text-[8px] font-normal text-emerald-300/80">Zero added walls, moved walls, or room geometry changes</span>
                                 </div>
                                </div>
                             </div>
                           )}
                           {(activeItem.is360 || (activeItem.dimensions && Math.abs(activeItem.dimensions.width / activeItem.dimensions.height - 2) < 0.2) || activeItem.assignedTools.some(t => t.startsWith('p360_'))) && (
                           ...
                           {activeItem.assignedTools.includes('wall_unifier') && (
                             <div className="space-y-3 border-t border-white/5 pt-4">
                               <div className="flex items-center justify-between">
                           ...
                           {activeItem.assignedTools.includes('floor_replacer') && (
                           ...
                           {activeItem.assignedTools.includes('ceiling_replacer') && (
                             <div className="space-y-2 border-t border-white/5 pt-4">
                      ...
                      <div className="space-y-4 pt-4 border-t border-white/5">
                          ...
                          {/* QUICK COST CONTROL & DAILY TOKEN BUDGET IN SIDEBAR */}
                          ...
                          <div className="text-[8px] font-medium text-slate-500 italic px-1">Note: Batch processing multiple images with Premium tools (Videos) will consume significantly more tokens/credits.</div>
                  ...
                  <div className="w-full lg:flex-grow relative" id="gallery-grid-container">
                    <MobileGalleryFastScroller 
                      items={displayedItems}
                      activeItemId={activeItemId}
                      onItemSelect={handleToggleSelect}
        ...
        {activeTab === 'ai-analyst' && <AIAnalyst items={items} persistedResults={aiAnalystResults} onResultsChange={setAiAnalystResults} />}
